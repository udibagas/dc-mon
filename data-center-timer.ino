...
const int PIN_FIRE = 12;

// INPUT
const int PIN_HI_PRESSURE = 13;
int nyala = 1;
...
void setup() {
    ...
    pinMode(PIN_PINTU_BELAKANG, INPUT_PULLUP);
    pinMode(PIN_PINTU_DEPAN, INPUT_PULLUP);
    pinMode(PIN_HI_PRESSURE, INPUT_PULLUP);

    // fan dan kompressor nyala di awal
...
void loop() {
    ...
    int pintuBelakang = digitalRead(PIN_PINTU_BELAKANG);
    int pintuDepan = digitalRead(PIN_PINTU_DEPAN);
    ...
    if (pintuBelakang == LOW || pintuDepan == LOW) {
        digitalWrite(PIN_LAMPU, HIGH);
    } else  {
